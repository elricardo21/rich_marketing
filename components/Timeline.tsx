...
const usps = [
  {
    icon: "🧬",
    title: "Obchodní DNA",
    desc: "5 let přímé akvizice, 10 000+ telefonátů, 1 000+ schůzek. Neřeším proklik — řeším marži a prodejní logiku.",
  },
  {
    icon: "⚙️",
    title: "Technická autonomie",
    desc: "Shoptet, tracking, UX, kódování — bez externího vývojáře. Díky AI zvládám úpravy v řádu hodin, ne týdnů.",
  },
  {
    icon: "🤖",
    title: "AI-Native Workflow",
    desc: "Vibecoding, Claude Code, automatizace. Technická řešení vznikají dny vs. týdny u agentur.",
  },
  {
    icon: "🎯",
    title: "Seniorita bez omáčky",
    desc: "Fakturuji expertízu a časovou úsporu. Profi výsledky v normálním jazyce — bez prázdných frází.",
  },
];
...
const steps = [
  {
    ...
    desc: "5 let v přímém prodeji, 10 000+ hovorů. Rozumím tomu, co klient potřebuje — protože jsem na jeho straně stál.",
    color: "#f95738",
    icon: (
...
export function Timeline() {
  return (
    <section className="py-24" style={{ backgroundColor: "var(--background-alt)" }}>
          ...
          {steps.map((step, i) => (
            <BlurFade key={step.number} inView delay={0.15 + i * 0.12}>
              <div className="relative flex flex-col items-center text-center group">
                {/* Node */}
                <div
                  className="relative z-10 w-24 h-24 rounded-2xl bg-card border flex items-center justify-center mb-6 transition-all duration-300"
                  style={{
      ...
      <BlurFade inView delay={0.65}>
        <div className="mt-16 text-center">
          <div className="inline-flex items-center gap-3 rounded-full border border-[#f95738]/40 bg-[#f95738]/10 px-6 py-3">
      ...
      <BlurFade inView delay={0.75}>
        <div className="mt-12 flex flex-wrap gap-8 justify-center text-sm text-muted-foreground">
          {[
            ["100+", "klientů v historii"],
            ["10 000+", "akvizičních hovorů"],
            ["5+", "let v obchodu"],
          ].map(([num, label]) => (
            <div key={label} className="text-center">
              <div className="text-2xl font-bold text-[#f95738]">{num}</div>
